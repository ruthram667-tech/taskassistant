...
// Endpoint to get all sent email logs
app.get("/api/email/logs", (req, res) => {
  res.json(sentEmailLogs);
});

// Endpoint to trigger a confirmation email on task setup
...
`;
    ...
    if (transporter) {
      ...
      if (ethereal) {
        ...
        previewUrl = nodemailer.getTestMessageUrl(sentInfo) || undefined;
        isMock = true;
        console.log(`[Info] Ethereal test email sent! Preview URL: ${previewUrl}`);
      } else {
        // Absolute fallback: mock log
...
// Track if we are in rate limit/quota cooldown to prevent spamming the API and logging errors
...
function handleRateLimitError(error: any): boolean {
  ...
  if (
    ...
    // Set rate limit cooldown for 2 minutes
    isRateLimitedUntil = Date.now() + 120 * 1000;
...
// Endpoint to generate smart, contextual productivity tips and motivational quotes
app.post("/api/gemini/tips", async (req: express.Request, res: express.Response) => {
  try {
...
`;
    ...
    // Clean response text from possible markdown wrappers if the model returned them
...
// Endpoint for the Task Assist Voice Agent Persona
app.post("/api/gemini/voice-agent", async (req: express.Request, res: express.Response) => {
  try {
    const { tasks, userName, userQuery } = req.body;
    const ai = getGeminiClient();

    const taskList = tasks || [];
    const highPriority = taskList.filter((t: any) => t.priority === "high" || t.priority === "urgent");
    const pending = taskList.filter((t: any) => t.status === "pending" || t.status === "overdue");
...
// Endpoint to analyze tasks and suggest an optimized Focus Queue
app.post("/api/gemini/focus-queue", async (req: express.Request, res: express.Response) => {
  try {
    ...
    const fallbackQueueResponse = {
      ...
      queue: taskList.map((t: any, index: number) => ({
        taskId: t.id,
        title: t.title,
...
// Helper function to provide high-quality fallback drafts if Gemini is offline or rate-limited
function getFallbackDrafts(prompt: string, channel: string, recipientType: string, tone: string, additionalDetails: string) {
...
// Endpoint to draft smart, contextual communications based on user wishes
app.post("/api/gemini/draft-message", async (req: express.Request, res: express.Response) => {
  try {
...
// Endpoint to generate 3-tier calling escalation audio scripts and automated voice message transcripts
app.post("/api/gemini/escalation", async (req: express.Request, res: express.Response) => {
  try {
    ...
    const chosenTier = tierDetails[escalationTier as 1 | 2 | 3] || tierDetails[1];
...
// Endpoint to generate highly relevant study guides and resources for any task
app.post("/api/gemini/task-reference", async (req: express.Request, res: express.Response) => {
  const { title, description } = req.body;
  const combinedText = ((title || "") + " " + (description || "")).toLowerCase();

  // Static Local Fallback Resources (100% reliable, zero API key quota usage, instantaneous)
  const fallbackData: Record<string, { topics: string[]; resources: { name: string; url: string; desc: string }[]; studyTips: string[] }> = {
    math: {
    ...
    cooking: {
      ...
      resources: [
        ...
        { name: "Bon Appétit Kitchen Hub", url: "https://www.bonappetit.com", desc: "Modern recipes, testing, kitchen tips, and step-by-step cook walkthroughs." }
      ],
      studyTips: [
  ...
  try {
    ...
    const ai = getGeminiClient();
    if (!ai) {
      // If AI Client not configured, return fallback immediately
      return res.json({ ...chosenFallback, aiGenerated: false });
    }
